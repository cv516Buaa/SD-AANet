...
def get_parser():
    parser = argparse.ArgumentParser(description='SD-AANet')
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    model = SDAANet(layers=args.layers, classes=2, zoom_factor=8, \
    ...
    optimizer = torch.optim.SGD(
        ...
        {'params': model.se_supp.parameters()},
        {'params': model.simple_mask.parameters()},
        {'params': model.s_cls.parameters()},
        {'params': model.q_maskgeneration.parameters()},
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (input, target, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        output, main_loss, aux_loss, kd_loss, simple_mask_loss = model(s_x=s_input, s_y=s_mask, x=input, y=target)

        if not args.multiprocessing_distributed:
            main_loss, aux_loss, kd_loss, simple_mask_loss = torch.mean(main_loss), torch.mean(aux_loss), torch.mean(kd_loss), torch.mean(simple_mask_loss)
        loss = main_loss + args.aux_weight * aux_loss + 50 * kd_loss + 0.5 * simple_mask_loss
        optimizer.zero_grad()
...
def validate(val_loader, model, criterion, epoch):
    ...
    for e in range(20):
        for i, (input, target, s_input, s_mask, subcls, ori_label) in enumerate(val_loader):
            ...
            #accuracy = sum(intersection_meter.sum) / (sum(union_meter.sum) + 1e-10)
            ...
            if args.use_coco:
                print_num = 1000
            if (iter_num % print_num == 0) and main_process():
                logger.info('Test: [{}/{}] '
                            'Data {data_time.val:.3f} ({data_time.avg:.3f}) '
                            'Batch {batch_time.val:.3f} ({batch_time.avg:.3f}) '
    ...
    if (class_miou > best_iou):
        best_iou = class_miou
        best_epoch = epoch
    logger.info('Best_mIOU:{}, Best_epoch:{}.'.format(best_iou, best_epoch))
