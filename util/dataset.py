...
def make_dataset(split=0, data_root=None, data_list=None, sub_list=None, use_split_coco=False):
    ...
    for l_idx in tqdm(range(len(list_read))):
        ...
        if use_split_coco:
            image_name = os.path.join(data_root, line_split[0][1:])
            label_name = os.path.join(data_root, line_split[1][1:])
        else:
...
class SemData(Dataset):
    def __init__(self, split=3, shot=1, data_root=None, data_list=None, transform=None, mode='train', use_coco=False, use_split_coco=False):
        ...
        if self.mode == 'train':
            self.data_list, self.sub_class_file_list = make_dataset(split, data_root, data_list, self.sub_list, use_split_coco=use_split_coco)
            assert len(self.sub_class_file_list.keys()) == len(self.sub_list)
        elif self.mode == 'val':
            self.data_list, self.sub_class_file_list = make_dataset(split, data_root, data_list, self.sub_val_list, use_split_coco=use_split_coco)
